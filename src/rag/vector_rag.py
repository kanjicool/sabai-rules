...
FEW_SHOT_EXAMPLES = """
[ตัวอย่างแนวทางการตอบที่ 1]
คำถาม: พนักงานระดับ 3 มีสิทธิ์ลาพักร้อนกี่วัน
คำตอบ: อ้างอิงจาก หน้า 18 (หมวด 4 วันลา และหลักเกณฑ์การลา):
...
[ตัวอย่างแนวทางการตอบที่ 2]
คำถาม: กองทุน PVD บริษัทสมทบให้อย่างไร
คำตอบ: อ้างอิงจาก หน้า 41–43 (หมวด 11 สิทธิประโยชน์สวัสดิการ):
...
[ตัวอย่างแนวทางการตอบที่ 3]
คำถาม: ทำงานมา 4 ปี ถ้าถูกเลิกจ้างจะได้เงินชดเชยกี่วัน
คำตอบ: อ้างอิงจาก หน้า 35 (หมวด 10 การเลิกจ้าง การพ้นสภาพ และการจ่ายค่าชดเชย):
...
class VectorRAGEngine:
    ...
    def query(self, user_question: str, top_k: int | None = None) -> RAGAnswer:
        ...
        # 3. Hybrid Search (Dense FAISS + Sparse BM25 + Re-ranking + Parent retrieval)
        hits = self.store.search(expanded_query, top_k=chosen_k, include_parents=True)

        # 4. Strict Guardrail: Relevance and Fallback Threshold
        ...
        # 5. Token Budget Management: pack context without overflowing
        context_text, included_hits, budget_used = self.budget_manager.pack_context(hits)

        # 6. Strict CoT System Prompt & Few-Shot Instruction
        system_prompt = (
            "คุณคือ HR Legal & Regulations Assistant ของ บริษัท พรีโม เซอร์วิส โซลูชั่น จำกัด\n"
            "จงปฏิบัติตามกฎเหล็กความถูกต้องอย่างเคร่งครัดดังต่อไปนี้:\n"
            "1. [Language]: ตอบคำถามเป็นภาษาไทยเท่านั้น ด้วยสำนวนที่เป็นทางการ สุภาพ ชัดเจน และจัดรูปแบบให้อ่านง่าย\n"
            ...
            "3. [Strict Grounding]: ตอบคำถามโดยอิงจาก 'เนื้อหาจากเอกสารข้อบังคับการทำงาน PRIMO' ที่กำหนดให้เท่านั้น ห้ามแต่งเติม ห้ามอนุมาน หรือเทียบเคียงจากภายนอกเด็ดขาด (Zero Hallucination)\n"
            "4. [Honest Fallback]: หากในเนื้อหาที่ให้ไม่มีตัวเลข เงื่อนไข หรือคำตอบที่ถาม ให้แจ้งตามตรงว่า 'ไม่พบข้อมูลระบุไว้ในข้อบังคับฉบับนี้' ห้ามเดาหรือสร้างข้อมูลขึ้นมาเอง\n"
            "5. [Chain-of-Thought & Citations]: ในคำตอบ ให้ระบุเลขหน้าและหมวดหมู่ที่พบข้อมูลเสมอ แล้วจึงสรุปสาระสำคัญ เช่น จำนวนวัน เงื่อนไข หรือขั้นตอน อย่างสมบูรณ์จบประโยค ห้ามตอบค้างครึ่งประโยค\n"
            "6. [Formatting]: ห้ามใช้ตาราง Markdown (เช่น |---|---|) หรือเครื่องหมาย ** ในคำตอบเด็ดขาด ให้สรุปเป็นรายการหัวข้อย่อยด้วยเครื่องหมาย • เพื่อให้อ่านง่ายบนโทรศัพท์มือถือ"
        )

        prompt = (
            f"=== ตัวอย่างแนวทางการตอบแบบมาตรฐาน (Few-Shot Examples) ===\n"
            ...
            f"คำสั่ง: กรุณาตอบคำถามโดยอ้างอิงจากเนื้อหาด้านบนตามแนวทางตัวอย่าง เป็นภาษาไทยที่ถูกต้องและชัดเจน โดยใช้เครื่องหมาย • สำหรับรายการย่อย:"
        )

        answer_text = self.llm.generate(prompt, system_prompt=system_prompt)

        # 7. Output Guardrail: foreign characters leakage detection & grounding verification
        ...
        # 8. Post-process: Convert any remaining Markdown tables/syntax to clean LINE bullets
